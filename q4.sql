...
create view avgOfFirst5Days as
	...
	from atLeast10Days join Dispatch on atLeast10Days.driver_id = Dispatch.driver_id
		join Dropoff on Dispatch.request_id = Dropoff.request_id
		join DriverRating on Dropoff.request_id = DriverRating.request_id
	where to_date(to_char(Dispatch.datetime, 'YYYY:MM:DD'), 'YYYY:MM:DD') >= to_date(to_char(firstDay, 'YYYY:MM:DD'), 'YYYY:MM:DD')
		and to_date(to_char(Dispatch.datetime, 'YYYY:MM:DD'), 'YYYY:MM:DD') < to_date(to_char(firstDay, 'YYYY:MM:DD'), 'YYYY:MM:DD') + 5
...
create view avgOfAfter5Days as
	...
	from atLeast10Days join Dispatch on atLeast10Days.driver_id = Dispatch.driver_id
		join Dropoff on Dispatch.request_id = Dropoff.request_id
		join DriverRating on Dropoff.request_id = DriverRating.request_id
	where to_date(to_char(Dispatch.datetime, 'YYYY:MM:DD'), 'YYYY:MM:DD') >= to_date(to_char(firstDay, 'YYYY:MM:DD'), 'YYYY:MM:DD') + 5
	group by atLeast10Days.driver_id, trained;

create view earlyAndLate as
	select avgOfFirst5Days.driver_id, avgOfFirst5Days.trained, early, late
	from avgOfFirst5Days full join avgOfAfter5Days on avgOfAfter5Days.driver_id = avgOfFirst5Days.driver_id;

create table train(type varchar(10));
insert into train values ('trained'), ('untrained');

create view finalResult as
	select case when trained then 'trained' else 'untrained' end as type, count(distinct driver_id) as number, avg(early) as early, avg(late) as late
	from earlyAndLate
	group by trained;
...
insert into q4
	(select type, case when number is NULL then 0 else number end as number, early, late from finalResult natural full join train);
