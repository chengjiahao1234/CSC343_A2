...
case when to_char(Request.datetime, 'YYYY') = '2014' then p1.location <@> p2.location else 0 end as distance_2014,
case when to_char(Request.datetime, 'YYYY') = '2015' then p1.location <@> p2.location else 0 end as distance_2015,
...
case when to_char(Request.datetime, 'YYYY') = '2014' then amount else null end as bill_2014,
case when to_char(Request.datetime, 'YYYY') = '2015' then amount else null end as bill_2015
...
INSERT INTO allMonth VALUES ('01'), ('02'), ('03'), ('04'), ('05'), ('06'), ('07'), ('08'), ('09'), ('10'), ('11'), ('12');


CREATE VIEW solutionOne AS
select D.driver_id, M.month, 0 as mileage_2014, 0 as billings_2014, 0 as mileage_2015, 0 as billings_2015, 0 as billings_increase, 0 as mileage_increase
from Driver D, allMonth M
where not exists 
(select driver_id, month from allRides
where D.driver_id = allRides.driver_id and M.month = allRides.month);


CREATE VIEW solutionTwo AS
select driver_id, month, sum(distance_2014) as mileage_2014, sum(bill_2014) as billings_2014,
sum(distance_2015) as mileage_2015, sum(bill_2015) as billings_2015, sum(bill_2015) - sum(bill_2014) as billings_increase,
